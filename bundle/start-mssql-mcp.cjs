...
`;
    ...
    function instanceToDiscover2(parts, parseDataSource2, { portOverride } = {}) {
    ...
    function withDiscoveredPort(parts, parseDataSource2, options = {}) {
      const target = instanceToDiscover2(parts, parseDataSource2, options);
    ...
    module2.exports = {
      ...
      instanceToDiscover: instanceToDiscover2,
      withDiscoveredPort
    };
  }
});
...
function readConnectionsFrom(file) {
  return path.extname(file).toLowerCase() === ".config" ? readWebConfigConnections(file) : readAppSettingsConnections(file);
}
...
${USAGE}`);
  ...
  try {
    if (source.kind === "env") {
      ...
      const configFile = source.kind === "configFile" ? resolveConfigFile(args.configFile) : null;
      const prepared = [];
      source.entries.forEach((entry, i) => {
        ...
        const rawParts = source.kind === "credentialsFile" ? entry.parts : source.kind === "flags" ? partsFromFlags(args) : parseAdoConnectionString(connString);
        prepared.push({
